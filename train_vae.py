    ...
    for epoch in range(1, args.epochs):
        for i, image in enumerate(tqdm(dataloader), 1):
            ...
            if (i+1)%50==0:
                print()
                print("#"* 50)
                print(f"Epoch: [{epoch}/{args.epochs}]\t Iter:[{i+1}/{len(dataloader)}]")
            ...
            if (i % args.save_freq == 0) or ((epoch == args.epochs-1) and (i == len(dataloader)-1)):
                with torch.no_grad():
                    preds = vae.reconstruct(16)
                im = vutils.make_grid(preds, padding=2).permute(1, 2, 0).numpy() * 0.5 + 0.5
                im = Image.fromarray((im*255).astype(np.uint8))
                im.save(os.path.join(args.save_dir, "reconstruction", f"epoch{epoch}_iter{i+1}.png"))
                vae.save_weights(os.path.join(args.save_dir, "weights", f"{epoch}_{i+1}.pth"))
